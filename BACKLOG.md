...
Local 9/2 (BUILD 20260831t): pad/mine arc strikeDamage always 4 (bats 6hp / slimes 4hp die in 1–2 taps; no iron / tray sword required); sword pad checked before MINE 1 OUT prefer so pad no longer steals exit; hurtPlayer i-frames 800→1200 after contact/block. Needs live. Household save intact. Never START OVER.
...
Last live pass: **2026-09-02 ~7:38–8:02am PT** BUILD 20260831s (desktop continued; prior `?v=pt0409` tab). Continued box Chrome save Day 1582 SPRING MORNING→EVENING (~11 min). Farmhouse → farm → woods/town (MARKET DAY) → MINE 1 → OUT to farm at MINE sign. **FIXED:** bat instant-KO gone — brief overlap −1 heart + knockback; survived multiple bats (no faint/−1G). **FIXED/confirm:** MINE 1 OUT one-tap exits. **WORSE/NEW:** sword pad knockbacks only (no kill/loot this pass); pad bottom-left edge can steal OUT (first pad tap exited mine). Sword STILL pad-only. Clock STILL races (MORNING→EVENING ~4 real min). START OVER STILL in Jobs. Tap-to-walk PASS. Slime contact UNTESTED. Never START OVER. Screenshots `/workspace/playtest-0738/`.

Highest-leverage theme: Local 9/2 BUILD 20260831t (Needs live): pad kill dmg + pad-over-OUT + longer i-frames shipped; sword STILL pad-only; clock STILL races; START OVER still under Jobs. Was LIVE 9/2 ~8am BUILD 20260831s: pad knockback-only; pad stole OUT; bat instant-KO FIXED but multi-bat drain harsh.
