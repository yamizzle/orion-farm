Local 9/26 (BUILD 20260905jx): Cloud City squirrel arc **hollow climb room**. As squirrel, darkForest far tree HOLE door-fades into `treeHollow` (N–S bark shaft; south → darkForest land 2-tile gap at 10,11; north UP stub toast THE TREETOPS ARE AHEAD. — no treetops walk/Moondrop reveal). Non-squirrel still TOO BIG TO FIT. Jobs CLIMB INSIDE THE HOLE. / CLIMB THE HOLLOW TREE. Reuse darkForest floors + code-paint bark (no new PNG). Save whitelist `treeHollow`. Autotest squirrelHoleProp / squirrelHoleTooBig / squirrelHoleAsSquirrel (enters hollow) / treeHollowWalk / treeHollowNorthTip / treeHollowExitForest / treeHollowSouthLand. Frozen DAY_MS/walk/Orion/grass0–3/evening/START OVER/oceanMap/peak rocks. Never START OVER. Kid-plain: turn into a squirrel, climb inside the hole in the far dark tree, then climb the hollow — treetops come later!

Local 9/25 (BUILD 20260905jw): Cloud City squirrel arc **hole toast** slice. darkForest far north darkGiantTree (7,8) marked `squirrelHole`; code-paint dark oval + floating HOLE label; tap/Space → TOO BIG TO FIT. (Orion) or A SQUIRREL HOLE! + climb-later dialogue (squirrel form). Jobs FIND THE TREE HOLE. / TRY THE HOLE IN THE TREE. Hollow climb / treetops **not** this ship. Autotest squirrelHoleProp / squirrelHoleTooBig / squirrelHoleAsSquirrel. Frozen DAY_MS/walk/Orion/grass0–3/evening/START OVER/oceanMap/peak rocks. Never START OVER. Kid-plain: as a squirrel, check the hole in the far dark tree — hard-refresh Safari first on iPad.

Local 9/24 (BUILD 20260905jv): Cloud City **Squirrel** wand spell (slice 1 — transform only). Fourth wizard-shop spell alongside fireball / lightning / bright light; buy/learn persists in `wandSpells.squirrel` + `unlocked.spell_squirrel`; Q cycles; hold wand + Space casts. Active form draws Orion as existing `Assets.squirrel` critter sprite (Orion base art frozen); ~12s duration or recast → off. Toasts: YOU TURN INTO A SQUIRREL! / BACK TO NORMAL. Jobs tip once when learned (TRY THE SQUIRREL SPELL.). Tree hole / hollow climb / treetops **not** this ship. Autotest spellSquirrelUnlock / spellSquirrelCast / spellSquirrelRecastOff. Frozen DAY_MS/walk/Orion/grass0–3/evening/START OVER/oceanMap/peak rocks. Never START OVER. Kid-plain: learn Squirrel at the Cloud City wizard — hold the wand and Space to turn into a squirrel for a bit!

LIVE 9/23 ~3:14–3:33pm PT (BUILD 20260905ju, box Chrome; single tab): farm Day 691 SPR PM → Day 692 SPR PM woods (job LEAVE WOOD AT NIM’S NOTE; 1G→2G after sword coin; 4/5♥). Clock **PASS** (PM→EVE→next-day PM; no multi-day jump). Bag dock / weapon dock (sword) / tray dense / birch·pine·oak 2× / no pink icon plates **STILL PASS**. Tray tools **STILL NEITHER** (seed/item icons; no shovel/axe/hoe/watering can). **FIXED:** Stars menu achievement medals no longer overlap labels (shot `02-stars-menu.png`) — verifies Local ju shrink. START OVER not pressed. Screenshots `/workspace/playtest-0923pm/`. Never START OVER.

LIVE 9/23 ~9:37–9:54am PT (BUILD 20260905jt, box Chrome; single tab): farm Day 652 SPR AM → Day 652 SPR PM (job LEAVE WOOD AT NIM’S NOTE; 1G; 4/5♥). Clock **PASS** (AM→PM, no day jump). Bag dock / weapon dock (sword) / tray dense / trees 2× / no pink icon plates **STILL PASS**. Tray tools **STILL NEITHER** (no shovel/axe/hoe/watering can — same mid-calendar profile). **NEW:** Stars menu achievement medals overlap and obscure several achievement labels (shot `stars-menu-overlap.png`). START OVER not pressed. Screenshots `/workspace/playtest-0923am/`. Never START OVER.

Local 9/23 (BUILD 20260905ju): Stars list medals shrink so labels read — LIVE 9/23 am NEW overlap (shot `stars-menu-overlap.png`). `drawAchieveList` was blitting full 32×32 `starBadge` into `rowH=11` rows (names at listX+20 sat under the medal). Fix: nearest-neighbor downsample to 10×10 (imageSmoothingEnabled=false, same pattern as HUD 05bd 16×16) + name at listX+14; trophy fallback scaled the same. Layout-only; no new art. Autotest achieveBadgeSmall. Frozen Orion/grass0–3/evening/DAY_MS/walk/START OVER/oceanMap. Never START OVER. Kid-plain: Stars medals are small again so you can read the names — hard-refresh.

Local 9/20 (BUILD 20260905jt): finish 05ja moss density — `assets/props/moss.png` was still 16×16 while High Peak craft siblings (branch/torch/torchUnlit/blueGlowMushroom) are 32×32. BOX/AREA densified soft green moss-clump pillow (bare item, no plate); drawBagKey + mossClump world draw 32→16 NN (imageSmoothingEnabled=false); pink gate 0 magenta. Autotest mossCraftIconDense. Frozen Orion/grass0–3/evening/DAY_MS/walk/START OVER/peak rocks/oceanMap/rowboat/atlas. Never START OVER. Kid-plain: moss looks crisp in the bag now — hard-refresh Safari first on iPad.
Local 9/19 (BUILD 20260905js): Mine 6 tray tools WORSE (9/18 smoke) — shovel/axe/pick missing from tray AND bag on late household save while sword dock stayed. Root: shovelOwned (and craft flags) can be false after pocket wipes even when dig holes prove the shovel was crafted; ensureOwnedTools only re-pins when the flag is true, and openBag never re-ensured. Fix: recover craft-tool ownership from tray/bag/held; one-time migrateRepairCraftTools restores shovelOwned from farmHole/woodsHole/hole or unlocked.shovelDigTip (no early-kit grant without dig proof); ensureOwnedTools + openBag re-pin owned tools; sword dock untouched. Autotest ownedShovelRestored / migrateShovelFromDigHole / migrateNoEarlyKitShovel. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: if you already dug holes, your shovel comes back — hard-refresh (Safari first on iPad).

LIVE 9/15 ~3:21–3:41pm PT (BUILD 20260905jr, box Chrome; single tab; hard-refresh): spawned PEAK (household save). **FIXED:** BAG dock on Peak (parchment + item grid + close; repeated opens) vs am 05jq Peak/farm bag WORSE and claimed 05jr chrome-over-hudWorldDoor. Clock **STILL WORSE** — Day 766 SPR NIGHT → Day 768 SPR NIGHT across ~20 min wall without sleep/faint (shots `01` Day 766 NIGHT → `04` mid → `08` Day 768 NIGHT). Weapon dock / tray dense / Peak pines 2× / no pink icon plates **STILL PASS**. Peak thin magenta world streaks **STILL** (snow). Mine Stairs 2 briefly reached. Farm / cabin / ocean / Junie / well / planting / woods UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0915-pm3/`. Never START OVER.

Local 9/15 (BUILD 20260905jr): HITL bag dock WORSE on Peak (LIVE 9/15 + box repro) — `tapHud` ran `hudWorldDoor` before BAG/dock/tray, so Peak cave/mouth/climb tiles under the bottom-right BAG chrome stole the click (chrome stayed; no grid). Fix: BAG/dock/tray/BUILD chrome win first; sword pad still first (pad-over-OUT KEEP); MINE 1 OUT prefer only after chrome. Autotest bagChromeBeatsPeakDoor. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: the bag opens again on the mountain — hard-refresh.

LIVE 9/15 ~9:24–9:45am PT (BUILD 20260905jq, box Chrome; single tab; hard-refresh): farm + cabin reach **PASS** (enter/stove/chest/bed/exit; 9/14 farm UNTESTED). Ocean transition **PASS**. Stars/Jobs **PASS** (BUILD 20260905jq). Weapon dock **STILL PASS**. Tray dense / trees 2× / no pink icon plates **STILL PASS** (no clear magenta world streaks this run). Clock **STILL WORSE** vs 9/14 pm WORSE-again — Day 723 SPR PM → Day 726 SPR EVE across ~20 min wall; Day 724 AM → Day 725 EVE in ~5m38s without sleep/faint (shots `01` 16:24Z Day 723 PM → `04` 16:30Z Day 724 AM → `05`/`06` mid → `07` 16:45Z Day 726 EVE). **WORSE:** BAG dock failed to open (chrome stayed; clicks no grid) vs 9/14 bag dock PASS. Junie mash = no dialogue (possible miss / not scored NEW). Well / kickball / planting / boat / Pip / Cloud City / mushroom climb / mine UNTESTED. Selecting tray item showed BUY A FISHING POLE AT PIP'S toast (expected tip). START OVER not pressed. Screenshots `/workspace/playtest-0915pm/`. Never START OVER.
...
Local 9/12 (BUILD 20260905jn): HITL wand BRIGHT LIGHT in darkForest white-washed the scene — `drawIlluminationOverlay` used `globalCompositeOperation = "screen"` with bright white radial after dayTint, and `illuminationActive()` stays true while wand+illumination selected. Fix: fold illumination into `drawDarkForestOverlay` offscreen cutout (same as torch 05jj — never dest-out on main): illuminationActive → large hole ~0.6 of max(VIEW_W,VIEW_H), 100% clear center → ~5% rim; else torchIsLitHeld medium; else tiny peek. `drawIlluminationOverlay` returns immediately in darkForest (no screen wash); outdoors soft warm source-over vignette at low alpha only. Keep illumUntil + continuous selected-wand light. Autotest: illumNoDarkForestScreen / darkForestIllumCutout. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: the wand’s bright light opens a bigger clear window in the dark forest — no white wash.

Local 9/12 (BUILD 20260905jm): Imagine **Cloud City** art stamp — Grok Imagine sources `assets/generated-src/cloud-city/*-src.png` → live props/ui/actors via magenta/hot-pink flood chroma + BOX/AREA downsample (cloudFloor opaque seamless AREA only). Shops 96×112, tiles/edge/wall dense 2×, icons 32×32, folk sheets sliced to nimbus/cirrus/cumulus/dew/breeze 32×48 facings (×2 walk frames). cloudFolk sprite ids dedicated; drawFolk Assets path; sceneCache artRev `05jm`. Pink-chroma gate PASS. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: Cloud City looks hand-painted and dreamy now — real shops and cloud people.

LIVE 9/12 ~3:05–3:32pm PT (BUILD 20260905ji, box Chrome; single tab): first live of 05ji (post 05je wall-ceiling harden; Pages still behind local 05jl). Clock **FIXED** vs 9/12 am 05jb WORSE — Day 240 SPR EVE → Day 243 SPR PM across ~27 min wall (≈ ~9 min/day); shot chain `01-hud-start` Day 240 EVE → `05-hud-mid` Day 241 PM → `09-hud-end` Day 243 AM → `11-hud-final` Day 243 PM; no multi-day hitch in shots. Agent noted possible same-minute Day 242 Night→243 AM without a Night shot — not scored WORSE. Junie **STILL PASS**; well splash **PASS** (am UNTESTED; COOL WATER. SPLASH!); weapon dock / bag dock **STILL PASS**; tray dense / trees 2× / no pink **STILL PASS**. Kickball not found; planting UNTESTED; mine / boat / cabin / Cloud City UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0912pm/`. Never START OVER.

Local 9/12 (BUILD 20260905jl): Moondrop Mountain **Cloud City** — Cap east (or cloudLedge east) → `cloudCity` (~30×16 ≈ 2× town plaza); west back to cap. Fluffy unwalkable cloud borders; walkable interior. Shops: Food (PUFF BUN / SKY JAM / FLUFFY SOUP / STAR COOKIE), Potion (+1 permanent max heart, cap +5 / `maxHpBonus`), Red Hat cosmetic overlay, Weapons (sword/bow/shield tier bumps gold+wood), Wizard (fireball / lightning / bright light spells). 5 townsfolk (NIMBUS/CIRRUS/CUMULUS/DEW/BREEZE) walk+talk. Cast: hold wand + Space (Q cycles spells). Jobs: VISIT CLOUD CITY on cap/ledge; shop tips in city. BOX dense 2× cloud tiles/buildings/icons; pink gate PASS. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: climb the giant blue mushroom to the clouds and explore Cloud City — shops, friends, upgrades, and wand spells.

Local 9/12 (BUILD 20260905jk): Moondrop Mountain **blue glow mushroom climb stages** — replace ~5ft/day linear with sleep growDays stages (always blue glow art). Day1 ≈ Orion H/ half W (solid prop, not climbable). Day2 ≈ 3×H / Orion W — climbable from south foot (Space/tap/walk); else Y-sort walk-behind (trunk tile only solid). Day3+ taller than yard view — climb from foot fades to **mushroomStalk** (N–S path, unwalkable cloud sides) → north **mushroomCap** → east stub **cloudLedge** toast `CLOUD CITY IS AHEAD.` (no Cloud City shops this BUILD). South stalk returns beside cabin garden plant. Jobs: climb the glowing mushroom / climb to the clouds. Persist plantDay/growDays (load no longer remaps blueGlow→carrot). BOX dense 2× stage1/2/3 + cap/cloud/stalk path art; pink gate PASS. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: the blue mushroom grows bigger each day — on day 2 you can climb it, on day 3 it reaches the clouds.

Local 9/12 (BUILD 20260905jj): HITL darkForest torch still lime-green glow — root cause: `drawDarkForestOverlay` painted black then `destination-out` **on the main game canvas**, erasing world pixels → transparent holes; CSS `#game` `background:#5FBF22` showed through as a green sphere. Fix: keep world intact; darkness on reused offscreen VIEW_W×VIEW_H (fill opaque black; dest-out radial on buffer only — lit medium ~1/3, 100%→~5% rim; unlit tiny faint peek); `source-over` drawImage buffer onto main. Never dest-out on main for this effect. No screen/lighter wash. Still requires `torchIsLitHeld` for big hole. Autotest: offscreen cutout / drawImage / no wash. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: the torch window shows the real dark forest now — not a green blob.

Local 9/12 (BUILD 20260905ji): HITL darkForest torch — black overlay over a **normal-colored** scene; lit torch = dest-out transparent sphere at **player** (Orion foot→center), 100% clear center → ~5% clear at rim, medium radius (~1/3 of view). Unlit = same punch, **tiny** faint peek (not pitch void). Removed screen/lighter/soft-light color wash (05jf amber `screen` was the lime sphere). Floors/trees stay 05jf readable under the hole. Autotest: no wash, follows player.x/y-cam, unlit tiny. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: the torch opens a clear window in the dark that follows you — no green wash.
Local 9/12 (BUILD 20260905jh): HITL torch graphics — crafted TORCH looked lit in tray/BAG because `drawBagKey` (~25874) treated `torch` and `litTorch` the same and always drew `Assets.torch` (flame); `Assets.torchUnlit` only if key literally `torchUnlit`. Fix: `torch`/`torchUnlit` → torchUnlit; `litTorch` → torch (flame) + tiny tray flicker when selected. Craft recipe icon `torchUnlit`; `drawItemIcon` handles standalone torch props for workbench/loot. Autotest draw path + craft icon. Kept dense 05ja 32×32 props (unlit wrap vs flame; pink gate PASS). Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: the torch looks dark until you light it on the stove, then it shows a flame.
Local 9/12 (BUILD 20260905jg): HITL High Peak tree chop silent no branch — `finishHarvest` already called `spawnGroundLoot("branch")` on highPeak, but `lootKindOk` (~9482) omitted `"branch"`/`"moss"` so spawn returned early (wood/pinecone still ok). Allow branch+moss in lootKindOk; stackKey/INV_KEYS/bag tips already had branch. Autotest lootKindOkBranch + highPeak pine smash → groundLoot/bag branch. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass/evening. Never START OVER. Kid-plain: chopping trees by the mountain cabin drops a branch again.
Local 9/12 (BUILD 20260905jf): HITL two bugs — (A) cabin sleep wake near farm house: `loadSave` pendingRestore whitelist missed `cabin`/`darkForest`/`wizardHouse` so sleep `persistSave(scene:cabin)` remapped to overworld keeping bed coords (~2,3). Fix: whitelist all three in outer if + ternary; cabin unwalkable restore → `sleepWakeSpot()`; autotest save→load stays in scene. (B) darkForest green-sphere void: floor avg~16 + tree avg~6 + 0.88–0.94 overlay. Restamped darkForestFloor/2 (needle/moss readable ~48–53) + darkGiantTree gray-brown bark contrast (avg~80, still 288×768/144×384); softer overlay (0.72 lit / 0.78 unlit) larger warm-amber cone; artRev `05jf` bust. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: sleep at the mountain cabin stays there after refresh; the dark giant trees and ground show up again in the torch light.
Local 9/12 (BUILD 20260905je): day-clock **race harden** (DAY_MS still frozen 480000). LIVE 9/12 am 05jb scored WORSE (Day 199 NIGHT→Day 200 AM ~51s shot gap; Day 199 EVE→Day 201 AM ~15 min — pace near ~8 min/day but mid-run flip looked jumpy). Fix: absolute `Date.now` wall ceiling (`clockWallOrigin`/`clockRankOrigin`/`clampClockToWall`) so poured rank cannot exceed wall elapsed + slack; multi-day `maybeAdoptStoredClock` jumps refused unless remote `savedAt` is clearly newer than `lastWrittenAt`; sleep day+1 re-arms ceiling. Autotest: clockWallArm / clockWallClamp / clockAdoptRefuseMultiDay / clockAdoptAllowFreshMultiDay. Critical day-clock stays Local/Partial — **Needs live verify**. Queued 9/10 cabin + giant-tree forest marked Done Local (05iz/05jb/05jd). Frozen walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: the day clock cannot jump ahead of real time anymore — hard-refresh.
LIVE 9/12 ~9:07–9:45am PT (BUILD 20260905jb, box Chrome; single tab): first live of 05jb (post 05ix FIXED + 05ja–05jd ships). Clock **WORSE** vs 9/11 pm FIXED — Day 199 SPR EVE → Day 201 SPR AM across ~18 min wall; mid-run Night→next-day AM flip within the same minute (shots `05-kickball.png` Day 199 NIGHT → `06-well.png` Day 200 AM). Junie **PASS**; weapon dock / bag dock **STILL PASS**; tray dense / trees 2× / no pink **STILL PASS**. Kickball / well splash / planting / mine UNTESTED this pass. START OVER not pressed. Screenshots `/workspace/playtest-0912am/`. Never START OVER.
Local 9/12 (BUILD 20260905jd): HITL Moondrop Mountain — restyle **darkForest** (giant tree grove) to Darren night-woods reference: scene near-pitch without litTorch (torch cone when held); `#` borders = absolute black void (no gray brick/houseShell); ground `y`/`f` = dark needle/moss floors (new darkForestFloor tiles); darkGiantTree draw **48×128 → 144×384 (~3×)**, Y-sort towers over Orion, clip outside map OK; restamped tall-trunk darkGiantTree.png 288×768 dense 2× (Imagine unavailable this agent — BOX/AREA extend from 05iz art + bark); sceneCache artRev `05jd` bust; clear fill black. Gameplay mouth/exit/moss/blue mushrooms/torch/path kept. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: the giant tree forest looks like a dark night woods — huge trees and black edges.
Local 9/12 (BUILD 20260905jc): HITL — after Peak summit rocks clear (`found.peakRocksCleared`), nest bird ride menu adds **BEAR** stop (keeps PEAK balcony). `getBirdStops()` builds from BIRD_STOPS + conditional bear; `finishBirdRide` bear → `applyScene("highPeak")` land south of bear choke (`LANDED BY THE BEAR.`) with walkable fallbacks; dialogue layout supports 7 picks (3-col multi-row); Jobs tip once `ASK THE BIRD TO THE BEAR`; highPeak cache invalidate null-guard. Autotest: no bear before rocks / has bear after / land near bear. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: after you clear the Peak rocks, the bird can fly you to the bear.
Local 9/12 (BUILD 20260905jb): HITL Moondrop Mountain cabin fixes — tap doormat/inndoor fell through to `goExitHouse()` (cabin missing from if/else); walk-onto `cabinOut` missing from `arrive()` (house has `isHouseExit`). Added cabin (+ wizardHouse) doormat branches → `goExitCabin` / `goExitWizardHouse`; `arrive()` cabin enter/exit + dark forest mouth/exit. `buildCabinProps` lean: KEEP bed/stove/workbench/houseChest/doormat/inndoor; REMOVE fireplace/windows/table/lantern/rug/crate/housePlant. Cabin sceneCache artRev `05jb` bust. Autotest: lean props, isCabinExit→highPeak walkable at cabinLand, tap src goExitCabin. Atlas 1477 keep. Frozen DAY_MS/walk/Orion/grass0–3/evening. Never START OVER. Kid-plain: cabin is just bed, chest, stove, and crafting table — and you can walk back outside again.
Local 9/12 (BUILD 20260905ja): graphics-style audit — High Peak craft icons blueGlowMushroom/torch/torchUnlit/branch were 1×/code-paint (bag fell through to fallback squares). Regened Imagine + BOX/AREA chroma 32×32; wired drawBagKey like wand; abandonedCabin draw 80×80→64×80 true 2×; moss left (Imagine→bush twice). Pink gate PASS. Frozen Orion/grass/evening/DAY_MS/swordfish-plate HITL untouched. Never START OVER. Kid-plain: cabin forest loot icons are crisp now — hard-refresh.
LIVE 9/11 ~3:20–4:30pm PT (BUILD 20260905ix, box Chrome; single tab): first live since 05iv multi-tab clock harden. Clock **FIXED** vs 9/10 pm 05ir WORSE — Day 73 SPR PM → Day 74 SPR PM across ~9 min wall (≈ OK vs ~8 min/day); end Day 75 Night; no rapid multi-day jumps. Farm kickball **STILL PASS**; well splash **STILL PASS** (COOL WATER. SPLASH!); weapon dock **STILL PASS** (sword left of tray); tray dense / trees 2× / no pink **STILL PASS**. Junie / planting / mine / grilled-golden icon UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0911pm/`. Never START OVER.

Local 9/11 (BUILD 20260905iz): High Peak north-bank redesign — Abandoned Cabin center (enterable: bed/stove/chest/craft), wizard house relocated west (wand+robes kept), east choppable trees, north dark gigantic-tree forest (torch needed). Map expand HIGH_PEAK 48×52→64×68 (~4×N / ~2×E/W of north bank). Cabin bed = save/sleep once cabin reachable (farm house bed stops). Torch craft branch+moss; light on stove; lit torch illuminates dark forest. Pick glowing blue mushroom → plant on cabin garden → grows ~5ft/day (persist growDays). Separate cabin + darkForest scenes. Jobs tips kid-plain. Restamped highPeakMap; dense 2× abandonedCabin/blueGlowMushroom/darkGiantTree/torch/branch/moss (atlas 1477 kept). Pink gate PASS. Bridge/bear/river south frozen. Never START OVER. Kid-plain: cross the bridge to Orion's mountain cabin — sleep there to save; craft a torch to explore the dark forest; plant the glowing mushroom by the cabin.

Local 9/11 (BUILD 20260905iy): High Peak river + fell-tree log bridge. Map taller north of bear (HIGH_PEAK_ROWS 36→52; bear ty 16→32). E–W river hard-blocks until axe chops the one riverside `bridgePine` → fall anim (~1s) → walkable `logBridge` + `found.logBridge` forever. Tall `peakGiant` grove scenery (not choppable). Wizard house on north bank past bridge (still needs bearFed/pathClear). Jobs: CHOP THE TALL TREE BY THE RIVER. / VISIT THE WIZARD HOUSE. Restamped highPeakMap (Imagine dirt/rock + water stamps, pink gate PASS); dense 2× peakGiant/bridgePine/logBridge standalone (atlas 1477 kept). Bear feed grilled golden unchanged. Never START OVER. Kid-plain: chop the tall tree by the river so it falls and makes a bridge to the wizard.

Local 9/11 (BUILD 20260905ix): HITL — grilled golden swordfish tray icon looked like plain cooked fish. New dense 32×32 atlas stamp (sword bill + wood plate + sparkles; pink-chroma PASS; no atlas repack). `paintFishIcon` keeps own icon + animated twinkle (never falls through to cookedFish). Grilled golden is **not eatable** (bear feed only); bag tip "FEEDS THE SLEEPY BEAR."; other cooked fish still edible. Frozen DAY_MS / walk / Orion / grass0–3 / evening. Never START OVER. Kid-plain: the grilled golden swordfish looks special with its sword and sparkles, and you save it for the sleepy bear — you cannot eat it.

Local 9/11 (BUILD 20260905iw): HITL — Darren fished ~75× from ocean pier with no golden swordfish (code path OK: pier → cast ocean "y" → waterKind ocean → roll). Bad luck plausible at 1/50, but story fish for the bear needs to be fairer. Ocean golden roll **1/50 → 1/20**; soft pity `found.oceanCatchesNoGolden` — after **25** ocean catches without golden, force next ocean catch to golden swordfish and reset. Farm ponds still blocked (`oceanOnly`). Save-safe; never START OVER. Frozen DAY_MS/walk/Orion/grass/evening. Kid-plain: ocean fishing still rare for the golden swordfish, but more fair, and it will show up if you’ve been unlucky a long time.

Local 9/11 (BUILD 20260905iv): multi-tab day-clock **save harden** (DAY_MS still frozen 480000 / ~8 min/day). LIVE 9/10 pm 05ir clock WORSE (Day 35→56 in ~5 min, then 87/89, mine Day 53 / end Day 55) with **four** Moondrop tabs open — single-tab hitch cannot explain that after 05ir gap-discard/budget. Fix: persistSave writes `savedAt`; before setItem adopt stored day/dayMs if ahead (`clockRank`); `storage` event syncs clock fields from other tabs (ahead always, or newer savedAt); updateClock only when visible **and** `document.hasFocus()`; blur/focus resetClockMark. Autotest: stale persist refused / ahead remote adopted. Critical day-clock reopened Local/Partial — **Needs live verify**. Never START OVER.

Local 9/11 (BUILD 20260905iu): Graphics-style audit — High Peak wizard house/NPC/wand/robes were code-BOX placeholders from 05it (Imagine unavailable). Regenerated with Grok Imagine + BOX/AREA chroma stamps at correct 2× (house 128×160, wand 32×32, NPC/robes 32×64); pink-chroma gate PASS; no atlas repack; frozen Orion/grass/evening/DAY_MS untouched. Hard-refresh. Never START OVER.

LIVE 9/10 ~3:30–4:50pm PT (BUILD 20260905ir session, box Chrome; 05is/05it shipped ~4:44–4:51pm after most checks): Clock **WORSE** vs 9/10 am FIXED on same 05ir — rapid multi-day jumps (Day 34 Night→Day 35 PM/Night OK ~10 min; Day 35 Night→Day 56 Eve in ~5 min; Day 56→Day 87 Eve in ~10 min; boat Day 89 Night; mine Day 53 Night after discontinuity; end Day 55 Eve). Day-digit HUD glitch on one crop. Junie / farm kickball / planting / well splash / bag+weapon dock / tray dense / trees 2× / no pink **STILL PASS**. Boat board+row **PASS** (am UNTESTED). Mine enter **PASS** (MINE 1 CAVE DOOR; am UNTESTED). Town kickball **PASS** (am UNTESTED). Chicken scoot UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0910pm/`. Never START OVER.

Local 9/10 (BUILD 20260905is): HITL fonts — tray/bag counts **smaller but sharper**: Press Start rasterize at **bake 24** (3× native 8) cream `#FFF8E8` + thin dark cardinal outline → hard-pixelize → NN blit `imageSmoothingEnabled=false` to on-screen **show 6** (exact 4:1; not soft 6px fillText). `PIXEL_COUNT_BAKE=24` / `PIXEL_COUNT_SIZE=6` / `TEXT_COUNT_SCALE=6`; tight per-digit tracking; no shadow. Talk boxes (`drawDialogue`) title/body/OK/X/choices route through `drawPixelText` **8px** Press Start (dark body on parchment + cream outline; cream+dark outline on wood tabs) — same clear letters as DAY bar; not legacy FONT fillRect. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER. Kid-plain: bag numbers are smaller but sharper; talk boxes use the same clear letters as the day bar.

LIVE 9/10 ~9:10–9:50am PT (BUILD 20260905ir, box Chrome): first live of 05ir clock harden. Clock **FIXED** vs 9/9 am 05iq WORSE — no rapid phase flips; Day 40 SPR Night → Day 43 SPR AM across ~34 min wall (≈ OK vs ~8 min/day). Junie **STILL PASS**; farm kickball **STILL PASS**; planting tilled **STILL PASS**; well splash **STILL PASS** (COOL WATER. SPLASH!); bag/weapon dock / tray dense / trees 2× / no pink **STILL PASS**. Ocean area entered **PASS**. Chicken scoot / boat board / town kickCart / mine UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0910am/`. Never START OVER.

Local 9/9 (BUILD 20260905ir): day-clock **race harden only** (DAY_MS still frozen 480000 / ~8 min/day Done). LIVE 9/9 am 05iq clock WORSE (rapid phase flips on input; tab discard/reload Day 149) — suspect visibility/background clockMark hitch catch-up. Fix: discard gaps >250ms (do not pour hitch as 50ms), real-time budget ≤1s game / 1s wall so rAF wake storms cannot outrun wall time, pause on visibilityState≠visible, freeze/resume reset clockMark, keep 50ms step cap + ≤1 day/tick + single-flight rAF. Autotest: hide→show / big dt no multi-day; step cap; DAY_MS/PHASE_MS unchanged. Critical day-clock row reopened as race Partial/Local (not day-length). Walk energy / Orion / grass0–3 / evening untouched. Never START OVER.

LIVE 9/9 ~9:05–9:50am PT (BUILD 20260905iq, box Chrome): Clock **WORSE** — rapid phase flips on input (Eve→Night→next-day AM across a short keyspam); after tab discard/reload household save landed **Day 149** (HUD screenshots). Mid-run also saw Day 36 Eve→Day 37 and a plant HUD at Day 151. Same BUILD as 9/8 pm where clock was OK (~8 min/day) — suspect visibility/background clockMark. Farm kickball **STILL PASS**; planting **PASS** (tilled yes / grass no); Junie talk **PASS**; bag/weapon dock / tray dense / trees 2× / no pink **STILL PASS**. Well / chicken / boat / town kickCart / mine UNTESTED (OCEAN label reached, no pier/boat on screen). START OVER not pressed. Screenshots `/workspace/playtest-0909am/`. Never START OVER.
...
Local 9/7 (BUILD 20260905io): HITL — tray/bag stack numbers still a bit too big at 8px. `drawSlotCount` / `PIXEL_COUNT_SIZE` / `TEXT_COUNT_SCALE` **6px** Press Start 2P cream `#FFF8E8` + thin dark cardinal outline + tiny shadow on wood slots. HUD sizes/colors/clock untouched (DAY/clock/BAG/place stay 8px white+outline; money gold; no energy E). World labels stay 8px white. Kid-plain: bag and tray numbers a bit smaller again (6px). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905in): HITL — tray/bag stack numbers too big. `drawSlotCount` / `PIXEL_COUNT_SIZE` / `TEXT_COUNT_SCALE` **8px** Press Start 2P (same as HUD/labels), still cream `#FFF8E8` + thin dark cardinal outline + tiny shadow on wood slots. HUD sizes/colors/clock untouched (DAY/clock/BAG/place stay 8px white+outline; money gold; no energy E). World labels stay 8px white. Kid-plain: bag and tray numbers are smaller now (8px). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905im): Locked **Press Start 2P** (SIL OFL, `assets/fonts/PressStart2P-Regular.ttf` + OFL.txt) via FontFace/`bustUrl` before HUD (boot `Promise.all` with Assets.load). Tray/bag `drawSlotCount` = **16px** cream `#FFF8E8` + dark thin cardinal outline + tiny shadow (removes 05hx 4×/0.85 — NOT Imagine, NOT 24px). World labels (`drawShaftLabel` / `drawDoorSignLabel` / woods words) = **8px white** + thin cardinal outline only (not 8-dir); OUT same (no gold). HUD DAY/clock/BAG/place = 8px white+outline; money gold; **no** energy "E" (bar only). `clockHudLine` short `SPR · AM|PM|EVE|NIGHT`. Crisp helper: integer fillText, smoothing off, AA→hard threshold. `pixelTextWidth` for new metrics; dialogue stays legacy FONT. Autotest drops 0.85 asserts. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905il): HITL — 05ig cream restore didn't work. Restored exact **05hx** text from `dce65cc`: plain 1:1 `drawText` / `drawTextPitch` FONT fillRect (pitch 6 / variable; NO outline glow, NO PixelFont). `drawSlotCount` = 05hx denser ~85% 4× offscreen (pitch 4 if label≥4 else 5; woodOut shadow then parch body; NN blit dw/dh·0.85; imageSmoothingEnabled=false). HUD DAY/BAG/gold/dialogue use plain drawText. denseText/paintFontGlyphs stubbed so nothing reintroduces glow. Kept post-05hx features (bag tip, walk-through signs, peak/bear). Kid-plain: tray numbers are the smaller dense digits from the bear/peak turn again. Autotest asserts drawSlotCount 0.85 + scale=4 and no PixelFont. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905ik): HITL reject 05ij — PixelFont atlas + 8-neighbor glow looked too large/chunky. Restored 05ig live text path (`a4e61f3`): `paintFontGlyphs` 1:1 binary FONT fillRect; `denseText` single +1,+1 dark shadow then cream body. Bypassed PixelFont atlas blit. TEXT_COUNT_SCALE/TEXT_HUD_SCALE=1; TEXT_COUNT_FILL=#FFF8E8. No 05ih fractional soft shrink. drawSlotCount / HUD DAY/BAG/gold / floats / dialogue share denseText. Autotest matches 05ig path (cream + outline-fail-closed; dropped PixelFont atlas asserts). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905ij): HITL redo font system from the ground up — letters are real game pixels now, not fuzzy browser text. PixelFont: A–Z / 0–9 / space / punctuation as binary grids; bake once into 4× atlas; drawGlyph/drawString with imageSmoothingEnabled=false, Math.floor positions, integer scale 1 or 2 only. Replaces drawText / dense count live paths (no fillText, no soft fractional shrink). Cream fill + dark outline for tray counts + HUD-on-wood; dialogue/BAG/floats share the same API. textWidth matches new metrics. Walk-through entrance labels kept. Compact HUD height OK. Autotest asserts smoothing stays off during text blit. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

LIVE 9/7 ~3:36–3:46pm PT (BUILD 20260905ii, box Chrome): first live of 05ii (HITL font/count ships after morning 05hw). Planting **PASS** (seed in tilled plot — morning UNTESTED). Well splash / Junie talk / bag dock / weapon dock **STILL PASS**. Tray icons dense, no pink plates; birch/pine/oak sharp 2× **STILL**. Clock **STILL OK** vs frozen DAY_MS (~2 min/phase): HUD Day 2 Morning→Evening→Night across ~7 min wall (screenshot mtimes); not WORSE (subagent race claim mistimed). Mine / chicken / boat UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0907pm/`. Never START OVER.

Local 9/7 (BUILD 20260905ii): HITL blurry font fix — stack counts / HUD / dialogue / BAG were soft vs crisp world pixels. Cause: 05ih 4× paint then fractional ~70% downscale under BUFFER=2 (non-integer device ratio) looked anti-aliased. Fix: disable image smoothing (NN only), snap draw positions to integers, only integer scale factors; ~70% snaps up to crisp 1:1 cream+outline that still reads. Never START OVER.

Local 9/7 (BUILD 20260905ih): denser smaller tray/bag stack counts — keep 05ig bright cream (#FFF8E8) + dark outline; paint digits at 4× then NN-downscale to ~70% on-screen. Outline+fill share one scratch (no 05if dest-+1 mush). Fail closed: outline pad grows so weight stays ≥2 device px at BUFFER=2. HUD DAY/gold/BAG/E stay 1:1 05ig path. Day panel ~21 kept. Walk-through entrance labels kept. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905ig): HITL 05if reject — tray counts illegible (tiny dark brown mush). Reverted stack counts to pre-05hx 1:1 bright cream (#FFF8E8) + 1px dark outline (no 0.85 NN crush). Unified HUD DAY/gold/SPRING/BAG/E + world floats onto that same paint path (shared denseText → paintFontGlyphs). Day panel ~21px (two full glyph rows; shorter than 05ie 23). Walk-through entrance labels kept. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905if): dense digit-style HUD — shared `denseText()` for HUD + tray counts (paint at 2×–4× then NN-downscale; light body + 1px dark outline). Dropped 05ie chunky near-1:1 fatten; TEXT_HUD_SCALE 0.75; stack counts stay TEXT_COUNT_SCALE 0.85. Shorter top wood HUD (day panel 15px) for more playfield. BAG tip / world floats / Jobs+Stars chrome use same path. Walk-through centered entrance labels kept. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905ie): HITL fonts readable after 05id densify. Shared crisp blit crops paint-pad (no hairline shrink); HUD scale 0.93 + dark shadow / light fill (DAY/gold/E/BAG/world floats same family as stack counts); counts stay smaller. Light text on wood; walk-through centered entrance signs kept. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Never START OVER.

Local 9/7 (BUILD 20260905id): denser smaller fonts + walk-through entrance labels. Shared crisp text blit (4× paint → NN scale, BUFFER-readable) for HUD/inventory/world signs (same path as bag count digits); slightly smaller so chips take less space. OCEAN/WOODS/MINE/PEAK/SHOP wayfinding words float centered on the door/ladder/cave tile (top-middle), not beside; entrance sign plaques no longer solid (oceanSign was blocking). OUT/UP/DOWN/MINE/PEAK shaft labels pinned (no bob / camera drift). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/peak art / bag tip 05ic / bear untouched. Never START OVER.

Local 9/7 (BUILD 20260905ic): BAG select tip — with BAG open, tapping a bag / tray-in-bag / dock cell shows a parchment strip under the grid: **NAME** + one short kid-plain sentence (what it does / where used). Tip follows the held drag item, updates on drop, clears on empty tap or bag close. Catalog covers tools/weapons/fish/food/ores/bars/seeds/kits/shards/cookedGoldenSwordfish/diamond pick + title-case fallback. Does not block X close or drag swap. Tray-only select outside bag unchanged. Text UI only (no new art). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/peak art untouched. Never START OVER.

LIVE 9/7 ~9:30am PT (BUILD 20260905hw stopwatch, box Chrome): Day clock races **Done** — ~2 min/phase / ~8 min/day matches frozen DAY_MS 480000; no residual race (9/6 audit). Never START OVER.

Local 9/7 (BUILD 20260905ib): PEAK path readable — restamped `assets/tiles/peakMap.png` from NEW Imagine warm brown dirt + calm pale snow-rock only (pink-chroma gate, BOX/AREA 16px stamps on 40×30; walk-mask aligned: dirt=`f` + summit under rock props cols 18–20 rows 2–5; off-path calm low-contrast snow-rock — NO busy striped dark tiles, NO mixed random tan/white checker). Smashable `peakRocks` props + pines kept (not painted-in). No scrub scars. sceneCache peak artRev=05ib bust. Leave High Peak / sleeping bear alone. Pink-chroma gate PASS (0 pink). No atlas 1× repack (height 1477 kept). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/chicken/canopy untouched. Never START OVER.

Local 9/7 (BUILD 20260905ia): PEAK ground rebuild — restamped `assets/tiles/peakMap.png` from NEW Imagine flat tiles only (dirt/stone path + snow/alpine rock; pink-chroma gate, BOX/AREA 16px stamps on 40×30; NO postage-stamp whole-scene peaks; NO baked smashable boulders; path dirt continuous UNDER summit rock props; scrub pillar + blue ice blob artifacts gone). Smashable `peakRocks` prop + crack stages (05bu) kept; `peakRocksCleared` still opens walk cols 18–20 rows 2–5 + summit mine mouth. Imagine pine props (not painted-in). Cave mouth / MINE enter unchanged. sceneCache peak artRev=05ia bust. Leave High Peak / sleeping bear alone. Pink-chroma gate PASS (0 pink). No atlas 1× repack (height 1477 kept). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/chicken/canopy untouched. Never START OVER.

Local 9/7 (BUILD 20260905hz): HIGH PEAK ground rebuild — restamped `assets/tiles/highPeakMap.png` from NEW Imagine alpine tileset + dirt-path (pink-chroma gate, BOX/AREA 16px flat rock/dirt on 48×36 grid; NO postage-stamp crops from peakMap; every cell filled, no checker holes). Walk mask unchanged (dirt=`f`, rock=`y`). sceneCache highPeak artRev=05hz bust so old map/props don’t stick. Leave sleepingBear.png alone (05hy PASS). Pine props + bear kept. Pink-chroma gate PASS (0 pink). No atlas 1× repack (height 1477 kept). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/chicken/canopy untouched. Never START OVER.

Local 9/7 (BUILD 20260905hy): true sleeping-pose bear — restamped `assets/props/sleepingBear.png` from Imagine magenta-chroma source (BOX/AREA dense 160×112, draw 80×56 BUFFER 2×); scrubbed #FF00FF / pink fringe; opaque body (~0.65); pink-chroma gate PASS (0 pink). highPeak draw scale / hitbox unchanged so bear still blocks path. Gameplay bear feed (grilled golden swordfish) unchanged. No atlas 1× repack (height 1477 kept). Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/chicken/canopy untouched. Never START OVER.

Local 9/7 (BUILD 20260905hx): HIGH PEAK look — restamped highPeakMap from Imagine peakMap/ocean dirt/stone tiles (16px grid, no code checker/triangles); sleepingBear BOX’d to dense 160×112 (draw 80×56, BUFFER 2×) from Imagine gen-try2 with magenta/green/blue keyed + stray scrub; existing Imagine pine props along the trail (not code triangles). Tray/bag stack counts draw 4× then blit smaller. Pink-chroma gate PASS (0 pink; bear opaque ~0.33; map 768×576). No atlas 1× repack (height 1477 kept). Gameplay: grilled swordfish bear gate / highPeak enter unchanged. Frozen DAY_MS / walk energy / Orion / grass0–3 / evening. Dock/boot/keepHeld/chicken/canopy untouched. Never START OVER.

LIVE 9/7 ~9:17–9:33am PT (BUILD 20260905hw, box Chrome): first live since 05hn — well splash **FIXED**/PASS (COOL WATER. SPLASH! + SPLASH banner; 05hb). Bag dock chrome **PASS** (dock on top + item grid + tray). Neighbor Junie talk **PASS**. Weapon dock **STILL PASS**; tray icons STILL dense/no pink plates; birch/pine/palm STILL sharp 2×. Clock **STILL** ~8 min/day (Day 2 Eve→Day 4 Morning across ~16 min wall; Day 3 Morning→Night ~6 min = 3 phases) — not WORSE; subagent 90s claim mistimed. Chicken / shovel-till / plant / mine / boat UNTESTED (tools tray empty this save). START OVER not pressed. Screenshots `/workspace/playtest-0907am/`. Never START OVER.

Local 9/6 (BUILD 20260905hw): bigger peak beyond summit mine — mountain-mine north cave (PEAK label) opens a larger HIGH PEAK scene (48×36 map, winding dirt path); large sleeping bear blocks mid-path with Z juice; talk/walk-in without food = sleepy dialogue; only GRILLED GOLDEN SWORDFISH (stove cook already wired; toast says GRILLED) feeds the bear, path clears, bearFed/pathClear persist; Jobs tips A BIGGER PEAK OPENS. / THE BEAR IS SLEEPY. / GRILL A GOLDEN SWORDFISH. Art: code-BOX sleepingBear + highPeakMap standalone PNGs (atlas height 1477 kept — no 1× repack / LOADING trap). Left alone: DAY_MS, walk energy, Orion, grass0–3, evening, dock/boot/keepHeld/chicken/canopy, summit mine enter/OUT-to-old-peak. Never START OVER.

Local 9/6 (BUILD 20260905hv): canopy peek harden — equal-foot side pines/oaks can hide Orion when Y-sort ties (old peek skipped `tf <= foot`); now only skip trees strictly behind, wider head/shoulder detect band, peekH 14→18 so kids always see head under dense Imagine canopy. No global +64. Sword pad / facing: code already omni after 05bz + weapon dock 05hm — no gameplay change; dock-selected facing-away pad autotest added. DAY_MS / dock / boot / keepHeld / chicken / summit / clockMark untouched. Never START OVER.

Local 9/6 (clock audit, no gameplay BUILD): dug Day clock races (05bq) end-to-end — DAY_MS still 480000 frozen; single scheduleLoop/rAF; updateClock only mutator (+sleep/faint/load); no scene-transition phase advance; no persistSave re-entry loop; visibility/pageshow reset clockMark; 50ms step cap + ≤1 day rollover/tick intact. Node sim: 15 real min @60fps ≈ Day N Morning→Day N+1 Night. LIVE 9/6 05hn Day 6 Morning→Day 7 Afternoon across ~15 min matches ~8 min/day (was mis-scored as race). Nest bird 10-wood→ride path intact (talkGiantBird / BIRD_STOPS / beginBirdRide). Autotest: DAY_MS/PHASE_MS + step-cap/hitch invariants. Needs live stopwatch (~2 min/phase) before Done — do not mark Done. Left alone: walk energy, Orion, grass0–3, evening color, dock/boot/keepHeld/chicken/summit. Never START OVER.

Local 9/6 (docs YOLO): close stale BACKLOG rows superseded by later ships — Car tiny → Done (05cc then 05hh/05hi size tune); Only one weapon tray → Done (05ay + 05hm dock, LIVE 9/6 05hn PASS); Island diamond pick → peak rocks → Done local+feature (05bt clear + 05hl/05hu summit mine); panel world-tap dismiss → Done live (05bc/05be PASS). Needs-live Criticals/Partials unchanged (day clock, canopy, sword pad). No gameplay BUILD. Never START OVER.

Local 9/6 (BUILD 20260905hu): summit mountain-mine polish — denser treasure room (6 crystals + silver/gold ore), stronger crystal glow/sparkle + GEM/BLUE GEM/GLOW CRYSTAL toasts + chest sparks; 3 openable chests with gems/ore/gold/bars + TREASURE! juice; mine carts look like real carts with CART float, tap/Space scoot CLANK!; OUT path clear; cache bust for pre-hu layouts. No full atlas 1× repack (code-paint carts). Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hl enter/OUT peak, 05ho–05ht dock/keepHeld/boot/chicken. Never START OVER.

Local 9/6 (BUILD 20260905ht): chicken opaque — HITL checkerboard/grass bleed through white feathers. Re-BOX'd Imagine hen sheet (05bd) with checker-bg flood + hole-fill + alpha harden (0 partial alpha); stamped chicken-right/left frames into dense atlas IN PLACE (256×1477 kept; no full 1× repack / no 05hn LOADING trap). Cheap birch pink-chroma scrub on props/inbox. artDensityOk still rock0.w≥32 height≥1200. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05ho–05hs bow/keepHeld/till/boot. Never START OVER.

Local 9/6 (BUILD 20260905hs): boot LOADING harden — `reloadForArt` returned void and still `location.replace`d when sessionStorage threw (private/blocked Chromebook storage), so a density miss could soft-reload forever and never `scheduleLoop` (05hn eternal LOADING class). Now returns false on miss/deny; Assets.load only early-returns when a reload actually started, else fail soft + boot. Also bumped stale `assets/version.json` (was stuck on 05hq). Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05ho–05hr bow/keepHeld/till. Never START OVER.

Local 9/6 (BUILD 20260905hr): grassTillDirt autotest — **not** a shovel-till regression. 05hi till code still tills plain farm grass→plantable dirt; the fail was leftover `player.swing` from the prior mound dig in the same autotest frame (kids wait for the shovel anim). Cleared swing + kept shovel selected so KEEP holds. Also cleaned three other autotest flakes (not live gameplay): fernSpaceCut / chairSpaceSit blocked by leftover dialogue/doorFade `uiPaused`; wellExists missing because spacePlantFlower had spliced the well at 20,28 out of overworld sceneCache. No gameplay code path changes for fern/well/chair. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hn–05hq selection/dock/boot. Never START OVER.

Local 9/6 (BUILD 20260905hq): placeable pick keepHeld — fence / lamp / crate / path / player-flower pick-up still **pin** wood/kit/stone/flower into the tray (05gq/gs/gu/gv) but no longer **steal** shovel/axe via selectTool mid-farming (canPickup* explicitly allows shovel/axe). Empty hand still auto-selects for TAP BUILD / TAP TILE. Shop / craft / chest TAKE / cook / smelt left intentional pin+select. Bow already fires without inventory ammo (projectile-only `arrows[]`) — no ammo gap. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hp dig/catch keepHeld, 05ho bow dock, 05hn boot/atlas. Never START OVER.

Local 9/6 (BUILD 20260905ho): weapon dock bow select — kids could not keep/arm the bow: forceSwordIntoTray always stomped the dock (openBag / load ensureOwnedTools / soft cave ensure), so sword won forever and dock tap only armed sword. Soft-ensure now leaves a parked bow and parks sword in the bag; hard pin only for cave claim / iron craft / giveTool sword; bag→dock drop selects the dock weapon; ensureOwnedTools no longer placeTool-swaps weapons over the saved dock. Sword dock select + pad/combat unchanged. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hn boot/atlas, 05hm dock chrome. Never START OVER.

LIVE 9/6 ~9:16–9:31am PT (BUILD 20260905hn, box Chrome): first live of 05hm/05hn — weapon dock **PASS** (sword in separate dock left of tray; tray = seeds only); tray icons STILL dense/no pink plates; birch/oak/palm STILL sharp 2×; pier boat **STILL PASS** (GET IN / row / BOAT STAYS HERE. / GET OUT). Clock STILL races (Day 6 Morning→Day 7 Afternoon). Bag open / mine / chicken / well / plant / mountain-mine UNTESTED. START OVER not pressed. Screenshots `/workspace/playtest-0916/`. Never START OVER.

Local 9/6 (BUILD 20260905hn): boot stuck on LOADING FARM forever — 05hm atlas pack shrunk dense 2× atlas (256×1455 → 256×1028) and rock0 32→16 so artDensityOk failed, reloadForArt ran once, then returned WITHOUT scheduleLoop (eternal green LOADING). Restored 05hl dense atlas + appended weapon-dock chrome (frame/empty/banner) on a new row without full 1× repack; harden boot to fail soft after one art reload. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hk perf, 05hl summit mine, 05hm weapon-dock logic. Never START OVER.

Local 9/6 (BUILD 20260905hm): weapon dock — one obvious sword/bow slot beside the tray (sword-frame chrome + empty WEAPON ghost); tray is tools/seeds/food only; picking a second weapon swaps into the dock and parks the old one in the bag; BAG open shows a big dock on top then the item grid (tray below = tools strip); dock tappable/selectable; number keys still pick tray 1–0; combat/sword pad/Space/bow feel unchanged; Pip buy equip + bag drag swap kept. Imagine unavailable this agent — dense 2× dock frame/empty/banner BOX’d into atlas (ASSET_REV=467520). Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05hk perf caching / offscreen cull / night wash, 05hl summit mine. Never START OVER.

REGRESSION / KEEP-FIXED checklist: see **REGRESSION.md** (HITL bugs Darren called out — skim before every ship; never revert without him).

Local 9/6 (BUILD 20260905hp): selected tool stays selected — ground loot / catch / collectible pickups still **pin** into the tray (05gn/gi/gr) but no longer **steal** shovel/axe/pick/pole/seeds/weapon via selectTool (dig→walk-on loot, chop wood, fish catch). Empty hand still auto-selects for tray-tap tips. Shop / craft / chest TAKE / cook / smelt / placeable pick-up (fence/kit/path/flower) still pin+select. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, 05ho bow dock, 05hn boot/atlas. Never START OVER.

Local 9/6 (BUILD 20260905hl): secret mountain mine on the peak — after the diamond pick clears the summit rocks, a cave mouth opens on the cleared path; walk/tap/Space enters a cozy mountain-mine with lootable crystals, decorative mine-cart crates, and openable treasure chests; OUT returns to the peak by the mouth; Jobs tip A SECRET MINE OPENS. / ENTER THE MOUNTAIN MINE. Save-safe found flags (mtGem/mtSapphire/mtCrystal/mtChest*). Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color, HUD tray/bag/weapon, 05hk perf caching / offscreen cull / night wash. Never START OVER.

Local 9/6 (BUILD 20260905hk): snappier Chromebook/iPad draw — evening/morning still BUFFER× crisp (05hg kept), but static trees/houses/rocks reuse graded scratch bitmaps until the day-color token or prop hurt-state changes (no per-frame rebuild); night actor wash uses multiply+mask instead of getImageData pixel walks; off-camera NPC/critter/mon draws skipped; lantern pop walks lights once per frame; Moondrop confetti spawn a touch calmer. Left alone: DAY_MS, walk energy, Orion art, farm grass0–3, evening color values, HUD tray, boat fish 05hj, car/shovel 05hi, peak cracks, combat, inventory, doors, pathing. Never START OVER.

Local 9/6 (BUILD 20260905hj): fish from the boat — aboard with the fishing pole selected, Space / tap ocean water casts (ocean fish + rare golden swordfish); without the pole selected, water taps still row (05h pole-steal stay fixed — no BUY/SELECT pole toast blocking the row). GET OUT / shore / inland sand row block untouched. 05hg BUFFER grade scratch + 05hi car/shovel untouched. Never START OVER.

Local 9/6 (BUILD 20260905hi): Reed car smaller + shovel tills grass — car draw ~0.75 of dense 3× frames (side ~54×36 ≈ a bit taller than Orion; 05hh sc1 still read house-huge); tap/lamps/shadow matched; beep/kick/Jobs kept. Shovel on farm grass makes plantable dirt again (05ax mounds-only + farmHole never setCell "p" broke expand plots); mound digs still loot; far plain-grass taps WALK (no whole-farm dig-steal); Space/near/mound dig; farmHole persists as dirt on reload. 05hg BUFFER grade scratch untouched. Never START OVER.\n\nLocal 9/6 (BUILD 20260905hh): Reed car size — keep denser 3× Imagine frames from 05cc but draw at scale 1 (side ~72×48 ≈ 1.5× Orion height; up/down ~48×60 ≈ 1.9×). 05cc scale 2 made the car house-tall (tire > Orion). Tap hitbox, night lamps, and ground shadow matched back to on-screen size; beep / kick-cart / Jobs / Reed unchanged. 05hg BUFFER grade scratch untouched. Never START OVER.
...
Local 9/6 (BUILD 20260905gd): Space peakSign adjacent — standing on or next to the MINE 6 / peak PEAK SIGN (facing away), Space / J / E finally climbs (tap already goMinePeakStairs / goPeakCave'd; cave/stairs ortho existed; sign tile is diagonal from mine stairs and south-of-mouth on peak so Space was a silent no-op). Never START OVER.
...
Local 9/6 (BUILD 20260905es): peak stairs + treetop exit landings — climb-down from cave stairs into MINE 6 no longer drops you one tile south of the UP stairs (22,6); land at (22,8) so a single north step cannot auto-reclimb; exit TREETOP onto TREE 4 lands at (4,4) (2 tiles from UP ty=2) matching other tree climb gaps. Never START OVER.
...
| Priority | Status | Item | Notes |
| --- | --- | --- | --- |
| High | Done Local 9/11 05iy | River + fell-tree bridge past the bear | **BUILD 20260905iy:** High Peak taller north; E–W river blocks until riverside giant falls as log bridge; wizard on north bank. Kid-plain: cut the big tree by the water so it becomes a bridge. **Needs live.** |
| High | Done Local 9/11–9/12 05iz/05jb | Abandoned cabin across the river | **BUILD 20260905iz + 05jb:** enterable abandoned cabin past log bridge (bed/stove/chest/craft); cabin bed is save/sleep; lean interior; exit doormat fixed. Kid-plain: cozy empty cabin becomes your new save bed. **Needs live.** |
| High | Done Local 9/11–9/12 05iz/05jd/05jk | Giant-tree forest + torch + blue mushroom climb stages | **BUILD 20260905iz + 05jd:** darkForest giant-tree grove + torch craft/light + blue glow mushroom pick/plant (~5ft/day). Night-woods restyle 05jd. Kid-plain: dark giant forest, torch, plant a glowing mushroom that grows huge each day. **Needs live.** |
...
| Priority | Status | Item | Notes |
| --- | --- | --- | --- |
| High | Local 9/10 05it | Wizard house past the High Peak bear | **Local BUILD 20260905it:** after feeding the bear, enterable **wizard house** on High Peak; talk wizard → **magic wand** parks in **weapon dock**; holding wand shows **wizard robes** overlay (Orion base art frozen). Save flags `found.wizardMet` / `found.wand` / `wandOwned` / `unlocked.wand`. Art: **05iu** Imagine BOX house/NPC/wand/robes (was code-BOX 05it); pink-chroma gate PASS; no atlas 1× repack. Kid-plain: past the bear, visit the wizard, get a wand, look like a wizard when you hold it. **Needs live.** |
...
| Medium | Done 9/6 local 05bt+05hl | Island cave Diamond Pickaxe → peak rocks | Done local+feature: **05bt** scrubbed painted pile + smashable prop / 10 diamond-pick hits open walk + peakRocksCleared; **05hl** secret mountain mine after clear (+ **05hu** treasure/cart polish). Island diamondPickaxe grant from earlier 05af/05bd kept. Household live smash→mine enter still welcome but feature shipped — not Partial. Never START OVER. |
...
| Medium | Done 9/6 05cc→05hh→05hi | Car is tiny | Town | Stale superseded: original tiny car closed by **05cc** denser 3× frames (scale 2 overshot house-tall), then **05hh** scale 1, then **05hi** ~0.75 draw (~taller than Orion) + matched hit/lamps/shadow; beep/kick/Jobs kept. Was Local 9/5 05cc needs-live / Partial tiny. NEW 8/24 Darren. |
...
| Medium | Done 9/6 live 05hm/05hn | Only one weapon equipped | Combat / HUD | Stale superseded: **05ay** one-weapon-in-tray → **05hm** weapon dock (sword/bow left of tray; tray = tools/seeds/food) + **05ho** bow select harden. LIVE 9/6 ~9:16am PT BUILD 20260905hn: weapon dock **PASS**. Was Local 9/5 05ay needs live. NEW 8/24 Darren. |
...
| Low | Done 9/5 7:10pm live | Closing a panel by tapping the world also walks | HUD | FIXED 9/5 ~7:10–7:23pm live BUILD 20260905be: panel world-tap dismiss **PASS** (05bc — Orion did not walk). Local 9/5 BUILD 20260905bc: ignoreWalkTap through pointerup + clear frozen path (bag/Jobs/Stars/talk/shop/craft/chest/notes/wipe). Was Open 8/21. |
